...
public class DefaultBuildToJSON implements BuildToJSON {
     ...
     * @throws IOException
     */
    public JSONObject getBuildData(Run run, String eventqStatus) throws IOException {
        JSONObject buildData;
        buildData = new JSONObject()
                .element("remote_id", String.valueOf(run.getNumber()))
                .element("event_time", convertTime(run.getTime()))
                .element("build_url", getBuildURL(run))
                .element("status",  getStatus(run, eventqStatus))
                .element("test_results", (Object) getTestResults(run)) // only include if not null
                .element("revisions", (Object) getRevisions(run)); // only include if not null

        // Jenkins doesn't update duration until after the build is complete.
        long duration = (System.currentTimeMillis() - run.getTimeInMillis());
    ...
    public JSONObject getRepositoryInfo(Run run) throws IOException {
        ...
        SCM scmServer = ((AbstractProject)run.getParent()).getScm();
        ...
        if ("hudson.scm.SubversionSCM".equals(serverType)) {
            ...
            repositoryType = "git";
            repositoryURI = getGitRepository(run);
        } else {
    ...
    protected URI getGitRepository(Run run) throws IOException {
        ...
        if (remoteUrls != null && remoteUrls.size() > 0) {
            ...
            SCM scm = ((AbstractProject) run.getParent()).getScm();
